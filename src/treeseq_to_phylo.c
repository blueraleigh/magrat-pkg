...
to_phylo1(tsk_tree_t *tree, tsk_id_t root, tsk_id_t *nodes, int *node_map)
{
    ...
    SEXP phy = PROTECT(Rf_allocVector(VECSXP, 5));
    SEXP phylo_names = PROTECT(Rf_allocVector(STRSXP, 5));
    SET_VECTOR_ELT(phy, 0, allocMatrix(INTSXP, k, 2));  // $edge
    SET_VECTOR_ELT(phy, 3, allocVector(REALSXP, k));    // $edge.length
    SET_VECTOR_ELT(phy, 1, allocVector(STRSXP, ntip));  // $tip.label
    SET_VECTOR_ELT(phy, 2, ScalarInteger(nnode));       // $Nnode
    SET_VECTOR_ELT(phy, 4, allocVector(STRSXP, nnode)); // $node.label
    ...
    SET_STRING_ELT(phylo_names, 4, Rf_mkChar("node.label"));
    ...
    for (j = 0, i = 0; j < num_nodes; ++j)
    {
        ...
        {
            SET_STRING_ELT(VECTOR_ELT(phy, 4), node_map[u]-ntip-1, 
                Rf_mkChar(node_label));
        }
        if (parent[u] == TSK_NULL || parent[u] == N)
            continue;
